...
class Filter(object):
    ...
    def write_gamma(self):
        default_text = open('gamma_template.i', 'r').read().split('*FLAG*')
        s = default_text[0]
        for d in range(self.params.num_disc):
            m = self.material_bank[self.materials[d]]
            args = [200 + d, m.num, -m.density, 130 + d, 131 + d, m.name]
        ...
        for m in set(self.materials + [0, 1, 2, 3]):
            ...
            s += 'c\n'
        s += default_text[5]
        with open('{}g.i'.format(self.ID), 'w+') as F:
            F.write(s)
    ...
    def extract_gamma(self):
        ...
        for i in range(len(keys)):
            ...
            for datum in data:
                ...
                d[keys[i]]['value'].append(float(dataStr[1]))
                d[keys[i]]['sigma'].append(float(dataStr[2]))
        self.g_g = d[21]['value'][4] + d[21]['value'][5]
